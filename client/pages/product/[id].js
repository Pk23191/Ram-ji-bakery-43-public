...
export default function ProductDetailPage() {
  ...
  const handleAddToCart = () => {
    ...
    addToCart(
      {
        ...product,
        // Fallback to Mongoose _id if id is missing
  ...
  if (isLoading) {
    return (
      <section className="section-shell py-16">
        <div className="glass-panel p-8 text-sm text-mocha/70">Loading product details...</div>
      </section>
    );
  }
  ...
  if (!product) {
    return (
      <>
        <Seo title="Product Not Found" description="Product details from Ramji Bakery." path={`/product/${id || ""}`} />
        <section className="section-shell py-16">
          <div className="glass-panel p-10 text-center">
            <h1 className="font-heading text-3xl text-cocoa">Product Not Found</h1>
            <p className="mt-3 text-sm text-mocha/70">This product is unavailable right now.</p>
    ...
    <>
      ...
      <section className="section-shell py-10 sm:py-14">
        <div className="mb-8 flex flex-wrap items-center gap-2 text-sm text-mocha/60">
          ...
          <Link href="/menu" className="hover:text-cocoa">
            Menu
          </Link>
          <span className="text-cocoa mx-2">/</span>
        ...
        <div className="grid gap-8 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="glass-panel p-6 sm:p-8">
            <div className="inline-flex rounded-full bg-latte px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-caramel">
              ...
              {(product.category || "Bakery Product").replace(/^./, (c) => c.toUpperCase())}
            </div>
            <div className="mt-5 flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="rounded-[24px] bg-white px-4 py-3 text-sm font-semibold text-cocoa shadow-soft">
                <div className="flex items-center gap-3">
                  ...
                  <span>{(averageRating || product.rating || 4.7).toFixed(1)}</span>
                </div>
            ...
            <div className="mt-8">
              <SectionHeader
                ...
                title="Freshly baked and delivered to your doorstep"
                description="Our bakery products are crafted with premium ingredients and delivered fresh daily across Dinara."
              />
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4 rounded-[28px] border border-white/60 bg-white/80 p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-mocha/70">Tell other customers how this bakery product looked and felt in person.</p>
                    </div>
          ...
          <div className="glass-panel p-6 sm:p-8">
            ...
            {isReviewsLoading ? (
              ...
              <div className="mt-6 rounded-[28px] border border-dashed border-caramel/30 bg-latte/20 p-8 text-center">
                ...
                <p className="mt-2 text-sm leading-6 text-mocha/70">Be the first customer to rate this bakery product.</p>
              </div>
            )}
          </div>
        </div>
